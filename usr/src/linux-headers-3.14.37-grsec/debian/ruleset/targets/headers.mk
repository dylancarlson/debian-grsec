...
debian/stamp/install/$(h_package):
	...
	$(make_prot_directory) $(TMPTOP)/lib/modules/$(KERNELRELEASE)
	ln -sf /usr/src/$(INT_STEM)-headers-$(KERNELRELEASE) $(TMPTOP)/lib/modules/$(KERNELRELEASE)/build
	$(eval $(which_debdir))
######################################################################
...
######################################################################
	$(install_prot_file) .config  	        $(SRCDIR)/.config
...
# XXX: restore this to add back the libc6 >= 2.14 dependency
# should perhaps do a make clean in the header clean code and move this after the hook
...
#	test ! -e debian/control~ || rm -f debian/control~
	...
	chmod -f 0500			    $(TMPTOP)/lib/modules/$(KERNELRELEASE)
