...
debian/stamp/install/$(i_package):
	...
	$(make_prot_directory) $(TMPTOP)/$(IMAGEDIR)
...
ifeq ($(strip $(KERNEL_ARCH)),um)
	$(install_prot_file) $(config)	 $(DOCDIR)/config-$(KERNELRELEASE)
else
	$(install_prot_file) $(config)	 $(TMPTOP)/$(IMAGEDIR)/config-$(KERNELRELEASE)
endif
...
# could have also said DEPMOD=/bin/true instead of moving files
	...
	test ! -f $(LGUEST_SUBDIR)/lguest ||			     \
	    $(install_prot_file) $(LGUEST_SUBDIR)/lguest $(TMPTOP)/lib/modules/$(KERNELRELEASE)/lguest
	test ! -f $(TMPTOP)/lib/modules/$(KERNELRELEASE)/lguest ||		   \
	    chmod 500 $(TMPTOP)/lib/modules/$(KERNELRELEASE)/lguest
      endif
    else
...
ifeq ($(strip $(delete_build_link)),YES)
	...
	rm -f $(TMPTOP)/lib/modules/$(KERNELRELEASE)/source
endif
	@echo done > $@
...
ifneq ($(strip $(image_clean_hook)),)
	...
	chmod -R og-rwx		       $(TMPTOP)
	chown -R root:root	       $(TMPTOP)
	chmod 0755		       $(TMPTOP)
	chmod -R og=rX		       $(TMPTOP)/DEBIAN
